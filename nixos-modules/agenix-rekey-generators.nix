{
  age.generators = {
    wireguard-priv = {pkgs, lib, file, ...}: let
    ...
    reality-keypair = { pkgs, lib, file, ... }: ''
      keys=$('${lib.getExe pkgs.sing-box}' generate reality-keypair)
      echo "$keys" | '${lib.getExe' pkgs.gawk "awk"}' -F ": " '$1 == "PublicKey" { print $2 }' > ${lib.escapeShellArg (lib.removeSuffix ".age" file + ".pub")}
