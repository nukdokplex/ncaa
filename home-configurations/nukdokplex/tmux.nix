{ config, lib, ... }:
let
  windowNumbers = builtins.genList (k: k + 1) 10;
  windowKeys = builtins.map (
    k: lib.last (lib.stringToCharacters (builtins.toString k))
  ) windowNumbers;
  windowShiftKeys = lib.stringToCharacters "!@#$%^&*()";
in
{
  programs.tmux = {
    enable = true;
    ...
    extraConfig =
      ''
        ...
        # --- GLOBAL OPTIONS ---
        set -s escape-time 0 # this disables tmux waiting key after pressing escape to make a combo, useful with vim
        set -s base-index 1 # numeration of panes, windows and sessions is starts from 1
        ...
        # goto window by number (creates window if doesn't exist)
      ''
      + builtins.concatStringsSep "\n" (
        lib.zipListsWith (
          windowNumber: key:
          "bind -n M-${key} if-shell 'tmux select-window -t :${toString windowNumber}' '' 'new-window -t :${toString windowNumber}'"
        ) windowNumbers windowKeys
      )
      + ''
      ...
      + builtins.concatStringsSep "\n" (
        lib.zipListsWith (
          ...
          "bind -n M-${windowKey} if-shell 'tmux join-pane -t :${toString windowNumber}' '' 'new-window -d -t :${toString windowNumber}; join-pane -t :${toString windowNumber}'"
        ) windowNumbers windowShiftKeys
      )
      + ''
