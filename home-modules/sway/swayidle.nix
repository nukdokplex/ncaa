...
in
{
  options.wayland.windowManager.sway.swayidle-timeouts =
    let
      mkTimeoutOption =
        name:
        lib.mkOption {
  ...
  config = lib.mkIf (cfg.enable && cfg.enableCustomConfiguration) {
    services.swayidle = {
      ...
      events = [
        {
          event = "before-sleep";
          command = "'${lib.getExe' systemd "loginctl"}' lock-session; '${lib.getExe pkgs.playerctl}' pause";
        }
        {
          event = "lock";
